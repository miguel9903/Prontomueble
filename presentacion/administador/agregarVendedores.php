...
if (isset($_POST['registrar'])) {  
    ...
    $vendedor = new Vendedor($_POST['cedula'], $_POST['nombre'], $_POST['apellido'], $_POST['correo'], $_POST['cedula'], $fecha, $fecha,  $_POST['telefono'], "1");
    
    if ($vendedor -> existeVendedor()) {
        $error = 1;
    }else{
        if ($administrador->existeCorreo() || $vendedor->existeCorreo()) {
            $error = 2;
        }else{
            $vendedor->insertar();
        }
...
<div class="modal fade" id="actualizarCliente" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
	...
	<div class="row mt-3">
		<div class="col-3">
			<div class="card ">
				<div class="card-header bg-primary text-white">Agregar vendedor</div>
				<div class="card-body">
					...
					<form method="post" action="index.php?pid=<?php echo base64_encode("presentacion/administador/agregarVendedores.php") ?>">
						<div class="form-group">
							<label>Cedula:</label> 
		...
		<div class="col-9">
			<div class="card">
				<div class="card-header bg-primary text-white">Buscar vendedores</div>
				<div class="card-body">
					<div class="form-group">
						<label>Filtro</label> 
						<input name="filtro" id="filtro" type="text" class="form-control" placeholder="Ingrese la cedula, nombre o apellido del vendedor" >
					</div>
				</div>
			</div>
	...
	function eliminarCliente(idCliente){
		alertify.confirm('¿Desea eliminar el cliente?', function(){ 
			$.ajax({
				...
				url: "indexAjax.php?pid=<?php echo base64_encode("presentacion/administador/eliminarVendedorAjax.php");?>",
				success: function(res){
				  alertify.success("Vendedor inhabilitado con exito");	
				}
			});
			}, function(){ 
...
$(document).ready(function(){
	$("#btnModificarCliente").click(function(){
		$.ajax({
			...
			url: "indexAjax.php?pid=<?php echo base64_encode("presentacion/administador/modificarVendedorAjax.php");?>",
			success: function(res){
...
$(document).ready(function(){
	$("#filtro").keyup(function(){
		if($("#filtro").val()!=""){
			var ruta = "indexAjax.php?pid=<?php echo base64_encode("presentacion/administador/buscarVendedorAjax.php"); ?>&filtro="+$("#filtro").val();
